...
public class TodoController {
    ...
    public ResponseEntity<?> createTodo(@RequestBody TodoDTO dto) {
        try {
            ...
            // (3) 임시 우저 아이디를 설정해준다. 이 부분은 4장 인증과 인가에서 수정할 예정이다. 지금은 인증과 인가 기능이 없으므로 한 유저(temporary-user)만 로그인 없이 사용 가능한 애플리케이션인 셈이다.
    ...
    public ResponseEntity<?> todoResponseEntity() {
        List<TodoEntity> todoDTOList = new ArrayList<>();
        ...
        // adding logic
        ResponseDTO<TodoEntity> response = ResponseDTO.<TodoEntity>builder().data(todoDTOList).build();
        return ResponseEntity.ok().body(response);
    }
}
